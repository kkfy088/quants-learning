...
export const quizzes: QuizQuestion[] = [
  {
    id: "q1", dayId: "crash-1",
    question: "量化分析的核心环节顺序是什么？",
    options: ["建模→收集数据→回测→部署", "收集数据→清洗→建模→回测→部署→监控", "回测→建模→收集数据→监控", "部署→监控→建模→回测"],
    answer: 1,
    explanation: "量化流程是：数据收集→清洗→建模→回测→部署→监控。跳过数据清洗直接建模是最常见的错误。",
  },
  {
    id: "q2", dayId: "crash-1",
    question: "供应链预测中，以下哪项不是常见的数据类型？",
    options: ["历史销量", "促销日历", "价格数据", "高管薪资"],
    answer: 3,
    explanation: "高管薪资与销量预测无关。历史销量、促销日历、价格、库存、节假日是核心数据类型。",
  },
  {
    id: "q3", dayId: "crash-2",
    question: "pandas 中 df['lag_7'] = df['sales'].shift(7) 的作用是？",
    options: ["把销量向后移动7天", "计算7天移动平均", "把销量向前移动7天", "删除前7行"],
    answer: 0,
    explanation: "shift(7) 把数据向后（向下）移动7行，lag_7[i] = sales[i-7]，即取7天前的值。",
  },
  {
    id: "q4", dayId: "crash-2",
    question: "数据清洗中，为什么缺货日不能填0？",
    options: ["0 会降低模型预测", "缺货≠需求为0，填0会导致截断偏误", "模型不支持0值", "缺少数据影响美观"],
    answer: 1,
    explanation: "缺货日销量为0不代表没有需求——是供应链断了。应标记为异常或用插值法估算真实需求。",
  },
  {
    id: "q5", dayId: "crash-3",
    question: "ADF 检验的 p-value=0.03，结论是？",
    options: ["序列不平稳，需要差分", "序列平稳，不需要差分", "无法判断", "模型过拟合"],
    answer: 1,
    explanation: "ADF 的 H0 是「序列不平稳」。p<0.05 拒绝 H0 → 序列平稳，不需要差分。",
  },
  {
    id: "q6", dayId: "crash-3",
    question: "Prophet 相比 ARIMA 的最大优势是？",
    options: ["预测更准", "速度快", "自动处理节假日/趋势变点/异常值", "代码更短"],
    answer: 2,
    explanation: "Prophet 自动处理节假日效应、趋势变点检测和异常值，适合业务人员快速上手。准确性取决于数据。",
  },
  {
    id: "q7", dayId: "crash-4",
    question: "为什么时序预测必须用 TimeSeriesSplit 而不是 KFold？",
    options: ["KFold 太慢", "时序有顺序，不能用未来数据验证过去", "TimeSeriesSplit 更简单", "没有区别"],
    answer: 1,
    explanation: "时序数据有先后顺序。KFold 随机划分会「用明天的数据预测昨天」——信息泄露。TimeSeriesSplit 保证验证集总在训练集之后。",
  },
  {
    id: "q8", dayId: "crash-4",
    question: "MAPE 和 WAPE 的主要区别是？",
    options: ["计算速度不同", "MAPE 各点等权重，WAPE 按销量加权", "WAPE 不能用于负值", "没有区别"],
    answer: 1,
    explanation: "MAPE 给每个预测点相同权重，低销量日误差会被放大。WAPE 按销量加权，更贴近业务关心的「总销量准不准」。",
  },
  {
    id: "q9", dayId: "crash-5",
    question: "Agent 框架中，最适合「数据→特征→建模→回测」有状态流水线的是？",
    options: ["CrewAI", "AutoGen", "LangGraph", "RD-Agent"],
    answer: 2,
    explanation: "LangGraph 的图式状态机最适合有分支/回退的复杂流水线。CrewAI 适合多角色协作，RD-Agent 专做因子挖掘闭环。",
  },
  {
    id: "q10", dayId: "crash-5",
    question: "供应链预测中，'缺货截断'问题的最佳处理方法是？",
    options: ["删除缺货日的数据", "把缺货日销量设为0", "标记缺货日并用模型估算反事实需求", "忽略这个问题"],
    answer: 2,
    explanation: "缺货日不代表真实需求。应标记为异常观测，用历史趋势或同类商品数据估算反事实需求，这是供应链预测最被低估的难点。",
  },
];
